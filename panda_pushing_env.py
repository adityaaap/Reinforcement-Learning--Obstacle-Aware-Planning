...
TARGET_POSE = np.array([0.8, 0., 0.])
OBSTACLE_CENTRE = np.array([0.55, 0.25, 0.])
OBSTACLE_HALFDIMS = np.array([0.05, 0.25, 0.05])


class PandaPushingEnv(gym.Env):

    def __init__(self, state_space='object_pose', reward_function=None, debug=False, visualizer=None,
                 include_obstacle=False, render_non_push_motions=True,
                 render_every_n_steps=1, camera_heigh=84, camera_width=84, img_width=32, img_height=32,
                 grayscale=False):
        """

        :param state_space: this can be either ['object_pose', 'image`]
        :param debug:
        :param visualizer:
        :param include_obstacle:
        :param render_non_push_motions:
        :param render_every_n_steps:
        :param camera_heigh:
        :param camera_width:
        :param img_width:
        :param img_height:
        ...
        self.init_panda_joint_state = np.array(
            [-0.00895896, -1.27076799, 0.00843109, -3.03526847, 0.00819568, 1.76413092, -2.36423618])
        ...
        p.resetDebugVisualizerCamera(cameraDistance=self.camera_dist_top, cameraYaw=self.camera_orn_top[0],
                                     cameraPitch=self.camera_orn_top[1],
                                     cameraTargetPosition=self.camera_pos_top)  # TODO: Remove
        ...
        self.object_pose_space = spaces.Box(
            low=np.array([self.space_limits[0][0], self.space_limits[0][1], -np.pi * 0.5]),
    ...
    def _get_observation_space(self):
        if self.state_space == 'image':
            if self.grayscale:
                observation_space = spaces.Box(low=0, high=255, shape=(self.img_height, self.img_width, 1),
                                               dtype=np.uint8)
            else:
                observation_space = spaces.Box(low=0, high=255, shape=(self.img_height, self.img_width, 3),
                                               dtype=np.uint8)
        elif self.state_space == 'object_pose':
            observation_space = self.object_pose_space
        else:
            raise NotImplementedError('State space {} not implemented, only implemented {}'.format(self.state_space,
                                                                                                   ['image',
                                                                                                    'object_pose']))
    ...
    def reset(self, object_pose=None, render_reset=True):
        ...
        self.objectUid = p.loadURDF(self.object_file_path, basePosition=object_pose[:3],
                                    baseOrientation=object_pose[3:], globalScaling=1.)
    ...
    def _is_done(self, state):
        ...
        at_goal = np.linalg.norm(object_pose[:2] - TARGET_POSE[:2]) < BOX_SIZE
    ...
    def push(self, push_location, push_angle, push_length=None):
        ...
        # set theta
        self.lower_down()
        self.planar_push(theta, push_length=start_gap - 0.015 - .5 * self.block_size,
    ...
    def get_state(self):
        if self.state_space == 'image':
            ...
            raise NotImplementedError(
                'State space {} not implemented, only implemented {}'.format(self.state_space,
                                                                             ['image', 'object_pose']))
        return state
    ...
    def _set_object_positions(self):
        ...
        object_target_pose_planar = TARGET_POSE
...
def default_reward_function(state, action):
    """
    Default reward function for the environment.
    :param state:
    :param action:
    :return:
    """
